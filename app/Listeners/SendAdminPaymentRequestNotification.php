...
class SendAdminPaymentRequestNotification implements ShouldQueue
{
     ...
     * @return void
     */
    public function handle(AdminPaymentRequest $event)
    {
        $project = $event->project;
        $paymentRequestStatus = $event->paymentRequestStatus;
        ...
        if($partTakers){
            foreach($partTakers as $token){
                if(isset($token['fcm'])){
                    ...
                    $fcm->setBody('Payment Request has been ' . $paymentRequestStatus . 'on '. $project['title']);
                    $fcm->setData($project);
